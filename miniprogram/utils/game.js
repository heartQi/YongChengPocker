...
function createGame(random = Math.random, difficulty = "normal") {
...
function legalAttackRanks(state) {
...
function canPlayerAttack(state, playerId) {
...
function hasLegalAttackCard(state, playerId) {
  return state.players[playerId].hand.some((card) => canAttackWith(state, playerId, card));
}

function canAttackWith(state, playerId, card) {
...
function canDefendWith(state, playerId, card, slotIndex = state.selectedDefenseSlot) {
...
function playDefense(state, playerId, cardId, slotIndex = state.selectedDefenseSlot) {
  const player = state.players[playerId];
...
function passAttack(state, playerId) {
...
function takeCards(state, playerId) {
...
function finishTakeCards(state) {
...
function canTakeCards(state, playerId) {
...
function canGiveCardWith(state, playerId, card) {
...
function completeDefense(state) {
  const defender = state.players[state.defenderId];
  const refillOrder = refillOrderForCurrentRound(state);
  state.battle = [];
...
function activeAttackers(state) {
...
function orderedAttackers(state) {
...
function nextAttackTurn(state, afterId) {
...
function advanceAttackTurnIfNoLegalCards(state) {
...
function nextOpponent(state, playerId) {
...
function nextPlayerOnTeamAfter(state, team, afterId) {
  for (let offset = 1; offset <= state.players.length; offset += 1) {
...
function chooseAiMove(state, playerId) {
  const player = state.players[playerId];
...
function applyAiMove(state, playerId) {
